...
public class Pathfinder : MonoBehaviour
{
    ...
    [SerializeField] float gridSize = 0.32f; //increase patience or gridSize for larger maps
    [SerializeField] float speed = 0.0125f; //increase for faster movement
    [SerializeField] int distance_danger = 5;
    [SerializeField] float distance_to_player = 0f;
    bool coll = true;
    ...
    void Start()
    {
        /*pathfinder = new Pathfinder<Vector2>(GetDistance, GetNeighbourNodes, 1);*/ //increase patience or gridSize for larger maps
    }

    private void Awake()
    {
        player = GameObject.Find("Player");
        pathfinder = new Pathfinder<Vector2>(GetDistance, GetNeighbourNodes, distance_danger);
    }

    // Update is called once per frame
    void Update()
    {
        if (Math.Abs(transform.position.y - player.transform.position.y) > 1)
        {
        ...
        {
            if ((Math.Abs(transform.position.y - player.transform.position.y) > distance_to_player || Math.Abs(transform.position.x - player.transform.position.x) > distance_to_player) && coll)
            {
